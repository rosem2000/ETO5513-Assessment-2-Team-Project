...
```{r busiest_locations, echo = FALSE, warning = FALSE, message = FALSE}
# Create Total_Peds for analysis
busiest_locations <- ped_data_megan1 %>%
  filter(Year == 2025) %>%
  group_by(Sensor_Name) %>%
  summarise(Total_Peds = sum(Total_Peds, na.rm=TRUE), .groups = "drop")
# Create a table of the busiest_locations arranged from highest to lowest and add caption
...
```
In Table \@ref(tab:busiest_locations) above, the ten sensors with the highest total pedestrian traffic in 2025 are displayed. The Swa31 sensor has the highest total traffic at **5,659,735** pedestrians in 2025, significantly greater than the tenth most popular sensor, Bou688_T at **2,608,494** pedestrians.
...
p <- ggmap(melb_map) +
  ...
  # Create axis labels and colour variation based on total pedestrians and dot size based on total pedestrians
...
# Save png to Figures folder
ggsave("Figures/pedestrian_map.png", plot = p, width = 10, height = 7, dpi = 300)
```
In Figure \@ref(fig:total-ped-map), it is evident that the busiest sensor location in Melbourne is the intersection of Swanston Street and Flinders Lane, recording over **5 million** total pedestrians in 2025.
