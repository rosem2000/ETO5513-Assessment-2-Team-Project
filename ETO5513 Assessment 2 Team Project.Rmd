...
# Create table of variable names, types and a brief description
knitr::kable(data.frame(
  Variable = names(ped_data_megan),
  # Ensure class is not returned as a matrix and is just returned as a single string per variable (OpenAI 2024)
  Type = sapply(ped_data_megan, function(x) paste(class(x), collapse = "/")),
  Description = c(
    "ID number for sensor location",
    "Date of observation",
    "Hour of the day in which observation was recorded",
    "Total number of pedestrians observed within the hour",
    "Name of sensor location",
    "Latitude and Longitude of sensor location"), row.names = NULL), caption = "Pedestrian Dataset Variables")
```
Within the subset of the Pedestrian Counting System data, there are **`r nrow(ped_data_megan)` observations** across **`r ncol(ped_data_megan)` variables**, as described in Table \@ref(tab:variables) above, with recordings dated from 8 August 2023 to 7 August 2025.
...
df <- read_xlsx("Data/ped_data.xlsx") 

# Creating new dataframe with only observations which are above 2 std dev of the average for that sensor and time
spike_df <- df %>%
  group_by(Sensor_Name, HourDay) %>%
  mutate(
    avg = mean(Total_Peds, na.rm = TRUE),
    sd  = sd(Total_Peds, na.rm = TRUE),
    spike_event = Total_Peds > avg + 2 * sd
  )

# Reducing data frame to dates of the top 15 observations above 2 std dev
top_spike_dates <- spike_df %>%
  filter(spike_event) %>%
...
# Plotting dates of these against the number of 'spikes' recorded
ggplot(top_spike_dates, aes(x = reorder(as.factor(Sensing_Date), spike_count), y = spike_count)) +
  geom_col(fill = "black") +
  coord_flip() +
  labs(
    title = "Top 15 Dates with Highest Pedestrian Spike Counts",
    x = "Date",
    y = "Number of Spikes"
  ) +
  theme_minimal()
...
# Creating dataframe with the standard deviation and mean of each sensor
variability_df <- df %>%
  group_by(Sensor_Name) %>%
  summarise(
    mean_count = mean(Total_Peds, na.rm = TRUE),
    sd_count   = sd(Total_Peds, na.rm = TRUE),
    .groups = "drop"
  )

# Get top and bottom 15 observations of standard deviation
top15 <- variability_df %>% arrange(desc(sd_count)) %>% slice_head(n = 15)
bottom15 <- variability_df %>% arrange(sd_count) %>% slice_head(n = 15)

# Combining top 15 and bottom 15 dataframes together
combined_df <- bind_rows(top15, bottom15) %>%
  arrange(sd_count) %>%
  mutate(Sensor_Name = factor(Sensor_Name, levels = Sensor_Name)) %>%
  pivot_longer(cols = c(mean_count, sd_count),
               names_to = "metric", values_to = "value")

# Plot
ggplot(combined_df, aes(x = Sensor_Name, y = value, fill = metric)) +
  geom_col(position = "dodge") +
  coord_flip() +
  scale_fill_manual(
    values = c("mean_count" = "orange", "sd_count" = "blue"),
    labels = c("Mean", "Standard Deviation")
  ) +
  labs(
    title = "Top 15 and Bottom 15 Sensor Locations by Volitility and Average traffic",
    x = "Sensor",
    y = "Count",
    fill = "Metric"
  ) +
...
Future studies should address current data gaps and incorporate additional factors such as weather variations or special events to more comprehensively model pedestrian behavior in Melbourne.
